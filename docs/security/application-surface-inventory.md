...
|---|---|---:|---|
| `security.architecture-application-surface.public-route` | Verified | 64 | Every Next.js page and route-handler method that permits an anonymous request according to the tested page and OpenAPI authentication registries. |
| `security.architecture-application-surface.authenticated-route` | Verified | 139 | Every Next.js page and route-handler method that requires a user, provider, internal, or signed-query identity according to the tested registries. |
| `security.architecture-application-surface.dynamic-route` | Verified | 69 | Every registered page or route-handler path with a dynamic segment. |
...
| `security.architecture-application-surface.rest` | Verified | 106 | Every source-discovered HTTP route-handler method. |
...
| `security.architecture-application-surface.websocket-event` | Verified | 69 | Every source-discovered Supabase publication/subscription event, RealtimeRefresh configured event, LiveKit RoomEvent handler, and LiveKit webhook event branch. Runtime delivery remains unverified. |
...
| `security.architecture-application-surface.database-function` | Verified | 61 | Every PostgreSQL function or procedure defined by tracked Prisma migrations and reviewed Supabase policy SQL, with application, migration-temporary, and Supabase Realtime database scopes kept distinct. |
...
| `security.architecture-application-surface.admin-cli` | Verified | 74 | Every package-exposed or directly executable script with source-static database, cloud, container, storage, worker, or HTTP state-change signals. Authorization and runtime side effects remain separately gated. |
...
| `security.architecture-application-surface.email-worker-search` | Verified | 4 | Every source-controlled mailto link, email SDK import, email sender call, and email worker declaration, including explicit scoped absence decisions. Provider-managed delivery is outside this proof. |
