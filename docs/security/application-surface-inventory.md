...
| `security.architecture-application-surface.admin-cli` | Verified | 72 | Every package-exposed or directly executable script with source-static database, cloud, container, storage, worker, or HTTP state-change signals. Authorization and runtime side effects remain separately gated. |
