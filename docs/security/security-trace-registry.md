...
| `security.final-summary-metric.missing-audit-events` | partial | 176 | Source endpoint records whose auditEvent array is empty. Not every operation requires an audit event and the required-event classification is incomplete. Observed facets: {"endpointRecordsWithoutLinkedAuditEvent":176,"traceRowsWithoutLinkedAuditEvent":12}. | `security/endpoints.ts`<br>`security/traces.ts`<br>`security/audit-events.ts` |
