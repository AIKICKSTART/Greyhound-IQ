...
- `package-lock.json` uses lockfile version 3 and currently inventories 1007 non-root lock entries: 31 direct runtime dependencies and 11 direct development dependencies. Those entries collapse to 958 unique package-name/version identities in npm's SBOM model.
...
- The lockfile-derived CycloneDX 1.5 application SBOM is checked in memory. All 958 unique lock identities must have matching name, version, package URL, SHA-512 hash and distribution URL, and all 959 dependency nodes must use known component references.
- The policy queries the npm advisory service through the approved npm registry and fails closed on an incomplete schema, inconsistent counts, a non-zero command exit, or any high or critical finding. The 2026-07-22 local run reported three moderate advisories and zero high or critical advisories after `fast-uri` was pinned to the patched 3.1.4 release.
...
- All 42 direct packages were reviewed against official npm registry metadata on 2026-07-22. No direct locked package is marked deprecated. A direct package with no maintainer or publisher fails the policy.
...
| The CI PostgreSQL service image becomes stale | `postgres:16` is pinned to a reviewed multi-architecture SHA-256 digest and uses synthetic credentials | Re-resolve and review the digest during scheduled dependency maintenance | No |
