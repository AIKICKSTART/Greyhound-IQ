...
`NOTIFICATION_WEBHOOK_URL` enables background delivery for persisted in-app notifications through `/api/internal/notification-delivery`. Leave it unset for in-app-only notifications. It must be one exact HTTPS URL without URL credentials, query parameters, fragments, or a custom port. Delivery pins DNS to public addresses and rejects redirects. `NOTIFICATION_WEBHOOK_SECRET` is mandatory when the URL is set and is sent only as `X-Notification-Secret` to that exact target. `NOTIFICATION_DELIVERY_MAX_ATTEMPTS` caps retry attempts before operator review.
