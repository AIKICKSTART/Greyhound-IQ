/**
 * Sequential supervisor for The Dogs raw dog-profile archive batches.
 *
 * It never starts a batch while another raw dog-profile worker is active. Before
 * each batch, it dry-runs the profile harvester to confirm there is pending work.
 *
 * Examples:
 *   npm run supervise:thedogs:dog-profile-raw
 *   npm run supervise:thedogs:dog-profile-raw -- --max-batches 6 --poll-ms 60000
 */
...
async function main() {
  ...
  console.log(
    JSON.stringify(
      {
        supervisor: "thedogs-profile-raw",
  ...
  for (;;) {
    if (options.maxBatches > 0 && batchesStarted >= options.maxBatches) {
      console.log(
        `[supervise:thedogs:dog-profile-raw] max batches reached: ${options.maxBatches}`
      );
      return;
    }
    ...
    if (active.length > 0) {
      console.log(
        `[supervise:thedogs:dog-profile-raw] waiting: ${active.length} dog-profile worker process(es) active`
      );
    ...
    console.log(
      `[supervise:thedogs:dog-profile-raw] pending probe: ${JSON.stringify(probe)}`
    );
    if ((probe.pendingProfiles ?? 0) <= 0 || (probe.selectedProfiles ?? 0) <= 0) {
      console.log("[supervise:thedogs:dog-profile-raw] no pending profiles remain");
      return;
    }

    batchesStarted += 1;
    const result = await runProfileBatch(options, batchesStarted);
    if (result.exitCode !== 0) {
      throw new Error(
        `dog-profile batch ${batchesStarted} exited with code ${String(result.exitCode)}`
      );
    }
  }
}
...
function parseOptions(args: string[]): Options {
  ...
  return {
    maxBatches: nonNegativeInt(stringOption(values, "max-batches"), 3),
    pollMs: positiveInt(stringOption(values, "poll-ms"), 60_000),
    limit: positiveInt(stringOption(values, "limit"), 2_000),
    ...
    progressFile: stringOption(values, "progress-file"),
...
async function probePendingProfiles(options: Options): Promise<PendingProbe> {
  const args = profileArgs(options, { dryRun: true, limit: 1 });
  const result = await runNpm(args, { capture: true });
  if (result.exitCode !== 0) {
...
async function runProfileBatch(options: Options, batchNumber: number) {
  const stamp = new Date().toISOString().replace(/[-:]/g, "").replace(/\.\d{3}Z$/, "Z");
  const outLog = path.join(LOG_DIR, `thedogs-dog-profile-supervised-${batchNumber}-${stamp}.out.log`);
  const errLog = path.join(LOG_DIR, `thedogs-dog-profile-supervised-${batchNumber}-${stamp}.err.log`);
  console.log(
    `[supervise:thedogs:dog-profile-raw] starting batch ${batchNumber}; logs: ${outLog}, ${errLog}`
  );
  const startedAt = Date.now();
  const result = await runNpm(profileArgs(options), { outLog, errLog });
  console.log(
    `[supervise:thedogs:dog-profile-raw] batch ${batchNumber} exited with code ${String(result.exitCode)} after ${Date.now() - startedAt}ms`
  );
  return result;
}
...
function profileArgs(
  ...
  const args = [
    ...
    "--",
    "--limit",
    String(override?.limit ?? options.limit),
    "--concurrency",
  ...
  if (override?.dryRun) args.push("--dry-run");
...
async function findActiveProfileWorkers(): Promise<ProcessMatch[]> {
  ...
  const script = [
    ...
    "Where-Object {",
    "($_.CommandLine -like '*backfill:thedogs:dog-profile-raw*' -or $_.CommandLine -like '*backfill-thedogs-dog-profile-raw.ts*')",
...
main().catch((error) => {
  console.error("[supervise:thedogs:dog-profile-raw] failed:", error);
  process.exitCode = 1;
});
