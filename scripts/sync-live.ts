 ...
 *
 * Usage: npm run sync:live -- [days:1-31] [upcoming|results|all]
 */
...
function daysArg() {
  const raw = process.argv[2] ?? process.env.LIVE_SYNC_DAYS ?? "31";
  const days = Number(raw);
  if (!Number.isInteger(days) || days < 1 || days > 31) {
    throw new Error("Usage: npm run sync:live -- [days:1-31] [upcoming|results|all]");
  }
  return days;
}
