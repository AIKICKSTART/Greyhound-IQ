...
assert.equal(structure.records.length, 97);
assert.equal(structure.records[0]?.timestamp, "20260630093000");
assert.equal(structure.records.at(-1)?.timestamp, "20260715110000");
...
assert.equal(nullability.createdTableColumns.length, 982);
assert.deepEqual(defaults.issues, []);
assert.equal(defaults.reviewedDefaults.length, 246);
assert.deepEqual(rewrites.issues, []);
assert.equal(rewrites.risks.length, 1);
assert.deepEqual(constraints.issues, []);
assert.equal(constraints.records.length, 327);
assert.equal(
...
assert.equal(indexes.records.length, 429);
assert.equal(
...
assert.equal(locks.records.length, 409);
...
console.log(
  "Migration forward-compatibility review passed: 97 ordered PostgreSQL migrations composed with 7 exact source audits covering 124 added columns, 246 defaults, 1 contract break, 1 rewrite, 327 constraints, 429 indexes and 409 ALTER TABLE operations.",
);
