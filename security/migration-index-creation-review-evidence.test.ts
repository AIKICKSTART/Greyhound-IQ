...
assert.equal(audit.occurrences, 469);
...
assert.equal(count("new-relation-immediate"), 401);
...
console.log(
  `Migration index-creation review passed: ${audit.records.length}/${audit.occurrences} indexes across ${sources.length} migrations classified; 401 new-relation, 1 concurrent and 67 exact pre-production immediate builds reviewed.`,
);
