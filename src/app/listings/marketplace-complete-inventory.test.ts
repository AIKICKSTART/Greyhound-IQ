...
assert.ok(pageSource.includes("not an active sale"));
assert.ok(pageSource.includes("Demo card · Preview only"));
assert.match(
  pageSource,
  /Custom cards for every dog you own are coming soon for Pro\s+members\./,
);
assert.match(pageSource, /as\s+a Pro membership benefit/);
