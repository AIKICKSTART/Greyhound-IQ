...
assert.equal(isInteractiveHelpTargetOversized(phoneViewport, hugeHomeTarget), true);
assert.equal(
...
const landscapeLayout = resolveInteractiveHelpPopupLayout(
  landscapePhone,
  null,
);
...
function assertPopupFits(
  ...
  const frameTop =
    viewport.offsetTop + (viewport.topInset ?? 0) + margin;
  const frameBottom =
    viewport.offsetTop +
    viewport.height -
