...
export const PRODUCT_BREAKPOINT_LINK_CRAWL_SCOPE =
  "Deterministic local source-crawl verification that all six discovered production-owned internal JSX links whose own or ancestor class source changes visibility at a responsive breakpoint remain included as exact route/source/target rows. The six source links belong to /feed and cover six internal targets; three carry link-level conditions, three inherit ancestor conditions, four rows include lg conditions, and two include md conditions. Responsive visibility requires an explicit breakpoint hide/invisible token or a base hidden/invisible token paired with a breakpoint reveal/display token. This closes only source-static inclusion of breakpoint-conditioned links; it does not prove rendered CSS, dynamic class values, browser viewport behaviour, destination behaviour, external links, deployed crawling, or production readiness.";
