...
export const PRODUCT_NON_NAVIGATION_LINK_SCOPE =
  "Deterministic local source-static proof that the production-reachable crawl includes all six non-navigation categories required by DISC.CRAWL.non-nav-links. The focused inventory records 143 category-tagged exact source rows: 18 footer target declarations, the six legal declarations within that footer set, seven login-return crawl rows, 110 dynamic-record route rows across 42 source files, one production mailto link, and one rendered notification-record link; the latter categories also retain their existing non-empty production-reachable interaction inventory checks. These rows come from footer, route, profile, account and feature components rather than a sitemap or top-navigation-only list. This closes only source inclusion for these six categories; it does not claim browser visibility, clickability, accessibility, destination HTTP status, redirect behavior, authentication-provider behavior, notification delivery, deployed crawling, or production readiness.";
