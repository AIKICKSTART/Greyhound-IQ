...
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /87 canonical route tours/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /ten redirect exclusions/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /does not prove the final deployed candidate/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /cross-device persistence/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /searchable help topics/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /recently completed tour history/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /help-focus intent selection/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /safe inline example/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /delayed-target attachment/i,
);
assert.match(PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE, /product area/i);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /server-resolved role/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /subscription tier/i,
...
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /never grant access/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /deterministic contrast measurement/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /eight explicit device-width policies/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /Visual Viewport keyboard bounds/i,
);
assert.match(PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE, /analytics/i);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /16,720-state base Design Lab preview matrix/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /1,040 explicit trainer\/support-operator/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /one-shot allowlisted non-mutating tab\/dialog reveal/i,
);
assert.match(
  PRODUCT_ONBOARDING_CAPABILITY_EVIDENCE_SCOPE,
  /nine consent-gated analytics events/i,
);
...
const onboardingRequirements = PRODUCT_MASTER_REQUIREMENTS.filter(
  ({ section }) => section.startsWith("onboarding."),
);
...
assert.equal(
  allTours.every(({ steps }) => steps.length === 5),
  true,
);
