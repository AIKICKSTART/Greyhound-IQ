...
export const PRODUCT_SOURCE_CRAWL_PROVENANCE_SCOPE =
  "Deterministic local source-crawl proof for 5,279 production-owned discovery rows: 2,242 internal-link rows and 3,037 interactive-action rows across the 89 production routes that contain a discovered link or action. Each row records its production owner route, source file, exact line and column, and nearest enclosing named source symbol (with an explicit module fallback). The /statistics route contains no discovered link or action and therefore contributes no provenance row. This closes only source-route and source-component provenance for the current static registry crawl; it does not claim visible or accessible labels, destination or redirect validity, action behavior, permissions, browser rendering, deployed crawling, external-provider behavior, or production readiness.";
