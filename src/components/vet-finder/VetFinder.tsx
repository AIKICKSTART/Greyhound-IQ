...
const VetMap = dynamic(
  () => import("./VetMap").then((mod) => ({ default: mod.VetMap })),
  {
    ssr: false,
    loading: () => <MapSkeleton />,
  },
...
const STATES = [
  ...new Set(VET_LOCATIONS.map((location) => location.state)),
...
export function resolveVetWorkspaceHeight({
  viewportTop,
  viewportHeight,
  workspaceTop,
  dockTop,
  ...
  const visibleTop = Math.max(workspaceTop, viewportTop);
  const visibleBottom = Math.min(viewportBottom, dockTop);
  return Math.min(
    800,
    Math.max(160, Math.floor(visibleBottom - visibleTop - 12)),
  );
}
...
export function VetFinder(): React.JSX.Element {
  ...
  const [filters, setFilters] = useState<VetFilterState>(() =>
    readFilters(new URLSearchParams(searchParams)),
  );
  const [basemap, setBasemap] = useState<BasemapMode>(() =>
    readBasemap(new URLSearchParams(searchParams)),
  );
  ...
  const selected =
    (selectedId &&
      (results.find((r) => r.id === selectedId) ??
        VET_LOCATIONS.find((v) => v.id === selectedId))) ||
    null;
  ...
  useEffect(() => {
    ...
    const resizeWorkspace = () => {
      ...
      animationFrame = window.requestAnimationFrame(() => {
        ...
        const viewportBottom = viewportTop + viewportHeight;
        const dock = document.querySelector<HTMLElement>(".giq-mobile-dock");
        const dockTop =
          dock && window.getComputedStyle(dock).display !== "none"
            ? dock.getBoundingClientRect().top
            : viewportBottom;
        workspace.style.height = `${resolveVetWorkspaceHeight({
          viewportTop,
          viewportHeight,
    ...
    resizeWorkspace();
    const resizeObserver = new ResizeObserver(resizeWorkspace);
    if (filtersRef.current) resizeObserver.observe(filtersRef.current);
    if (workspace.parentElement)
      resizeObserver.observe(workspace.parentElement);
    window.addEventListener("resize", resizeWorkspace);
    window.addEventListener("scroll", resizeWorkspace, { passive: true });
    window.addEventListener("orientationchange", resizeWorkspace);
    ...
    return () => {
      ...
      window.removeEventListener("scroll", resizeWorkspace);
      window.removeEventListener("orientationchange", resizeWorkspace);
      window.visualViewport?.removeEventListener("resize", resizeWorkspace);
      window.visualViewport?.removeEventListener("scroll", resizeWorkspace);
  ...
  return (
    <section className="relative mx-auto w-full max-w-[1600px] px-4 py-6">
      <div ref={filtersRef} className="giq-panel giq-vet-search-panel mb-4 p-4">
        <VetFilters
          filters={filters}
      ...
      <div className="mb-3 flex items-center justify-between gap-3 lg:hidden">
        <p
          className="text-[13px] font-semibold text-[hsl(var(--muted-foreground))] tabular-nums"
          aria-live="polite"
        >
      ...
      <div
        ref={workspaceRef}
        data-vet-workspace
        className="grid h-[min(640px,calc(100dvh-16rem))] min-h-40 overflow-hidden rounded-2xl border border-[hsl(var(--border))] shadow-[0_22px_60px_-24px_hsl(0_0%_0%/0.7)] lg:grid-cols-[minmax(300px,360px)_1fr]"
      >
        <div
        ...
        >
          <div className="flex min-h-[52px] items-center justify-between gap-3 border-b border-[hsl(var(--secondary)/0.35)] px-4 py-3">
            <h2
              className="text-[14px] font-semibold text-[hsl(var(--foreground))] tabular-nums"
              aria-live="polite"
            >
              {results.length} clinic{results.length === 1 ? "" : "s"}
            </h2>
        ...
        <div
          className={`giq-vet-map-shell relative min-h-0 ${mobileView === "list" ? "hidden lg:block" : "block"}`}
        >
          <VetMap
            locations={results}
            selectedId={selectedId}
...
function MapSkeleton(): React.JSX.Element {
  return (
    <div className="flex h-full w-full items-center justify-center bg-[hsl(var(--surface-1))]">
      <Loader2
        className="h-6 w-6 animate-spin text-[hsl(var(--primary-bright))]"
        aria-hidden="true"
      />
